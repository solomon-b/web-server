    ...
    overrides = hfinal: hprev: {
      ...
      hasql-interpolate = final.haskell.lib.dontCheck (
        hfinal.callCabal2nix
          "hasql-interpolate"
          "${inputs.hasql-interpolate-src}"
          { }
      );

      htmx = final.haskell.lib.dontCheck (hfinal.callHackageDirect
        {
          pkg = "htmx";
          ver = "0.1.0.0";
      ...
      tmp-postgres = final.haskell.lib.dontCheck (hfinal.callCabal2nix "tmp-postgres" inputs.tmp-postgres-src { });
    };
  };
}
