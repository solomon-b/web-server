...
type Route =
  "user"
    ...
    :> Servant.PostAccepted '[HTML] (Servant.Headers '[Servant.Header "Set-Cookie" (Maybe Text), Servant.Header "HX-Redirect" Text] Servant.NoContent)
...
handler ::
  ...
  m
    ( Servant.Headers
        '[ Servant.Header "Set-Cookie" (Maybe Text),
  ...
  execQuerySpanThrow (Session.getServerSessionByUser (User.mId user)) >>= \case
    Nothing -> do
      Auth.login (User.mId user) sockAddr mUserAgent >>= \case
        ...
        Right sessionId -> do
          pure $ Servant.addHeader (Just $ Auth.mkCookieSession sessionId) $ Servant.addHeader redirectLink Servant.NoContent
    Just session ->
      let sessionId = Session.mSessionId session
       in pure $ Servant.addHeader (Just $ Auth.mkCookieSession sessionId) $ Servant.addHeader redirectLink Servant.NoContent
...
invalidCredentialResponse emailAddress details = do
  ...
  pure $ Servant.addHeader Nothing $ Servant.addHeader ("/" <> Http.toUrlPiece (userLoginGetLink Nothing $ Just emailAddress)) Servant.NoContent
