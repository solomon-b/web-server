...
<div class='flex flex-col border border-gray-300 rounded-lg'>
    <div class='flex mb-2'>
        <div class='p-2 border-r border-gray-300 rounded-t-lg bg-white text-gray-900'>
          <button role='tab' hx-get="/blog/new/edit" hx-swap="innerHTML" hx-target="\#content-field" hx-include='next textarea'>
            Write
          </button>
        </div>
    ...
    <div class='m-2 min-h-60'>
        <textarea name='content' placeholder='Add your content here...' rows='11' class='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500'>#{fold content}</textarea>
    </div>
</div>
|]
