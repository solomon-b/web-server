...
headingButton =
  let onClick :: Text
      onClick = [i|x-on:click="textareaContent += '\#\#\# '; $refs.textareaContent.focus()"|]
   in [i|<i #{onClick} title='Heading' class='p-1 px-2 rounded-lg hover:bg-gray-200 fa-solid fa-heading'></i>|]

-- TODO: Move cursor to middle of bold section
boldButton :: ByteString
boldButton =
  let onClick :: Text
      onClick = [i|x-on:click="textareaContent += '****'; $refs.textareaContent.focus(); const pos = $refs.textareaContent.value.length - 2; $refs.textareaContent.setSelectionRange(pos, pos);"|]
   in [i|<i #{onClick} title='Bold' class='p-1 px-2 rounded-lg hover:bg-gray-200 fa-solid fa-bold'></i>|]

italicButton :: ByteString
italicButton =
  let onClick :: Text
      onClick = [i|x-on:click="textareaContent += '__'; $refs.textareaContent.focus(); const pos = $refs.textareaContent.value.length - 2; $refs.textareaContent.setSelectionRange(pos, pos);"|]
...
urlButton =
  let onClick :: Text
      onClick = [i|x-on:click="textareaContent += '[](url)'; $refs.textareaContent.focus(); const pos = $refs.textareaContent.value.length - 2; $refs.textareaContent.setSelectionRange(pos, pos);"|]
   in [i|<i #{onClick} title='Link' class='p-1 px-2 rounded-lg hover:bg-gray-200 fa-solid fa-link'></i>|]

-- TODO: Insert at beginning of line:
numberedListButton :: ByteString
numberedListButton =
  let onClick :: Text
      onClick = [i|x-on:click="textareaContent += '1. '; $refs.textareaContent.focus(); const pos = $refs.textareaContent.value.length - 2; $refs.textareaContent.setSelectionRange(pos, pos);"|]
...
contentField content =
  [i|
<div id='content-field' x-data="{ textareaContent: '#{fold content}' }">
  ...
  <div class='flex flex-col border border-gray-300 rounded-lg'>
      ...
      <div class='m-2 min-h-60'>
          <textarea name='content' x-model="textareaContent" x-ref="textareaContent" placeholder='Add your content here...' rows='11' class='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500'>#{fold content}</textarea>
      </div>
  </div>
</div>
|]
