...
template :: [(BlogPosts.Domain, User.Domain)] -> SearchForm -> Lucid.Html ()
        ...
        $ do
          ...
          select_
            ...
            $ do
              ...
              let authors = nub $ User.dDisplayName . snd <$> posts
          ...
          tr_ [class_ "border-t hover:bg-gray-50"] $ do
            ...
            td_ [class_ "px-4 py-3 font-medium text-gray-900"] $ do
              button_ [hxGet_ [i|/#{blogEditUrl dId}|], hxTarget_ "#admin-main", hxPushUrl_ "true"] $
                Lucid.toHtml $
                  display dTitle
            td_ [class_ "px-4 py-3"] $ Lucid.toHtml $ display $ User.dDisplayName user
...
handler (Auth.Authz user@User.Domain {..} _) hxTrigger searchQuery (flatten -> startDate) (flatten -> endDate) (flatten -> author) = do
  Observability.handlerSpan "GET /admin/blog" () (display . Servant.getResponse) $
    if dIsAdmin
      then do
        postsWithAuthors <- do
          allPosts <- fmap (\(x, y, z) -> (BlogPosts.toDomain (x, y), User.toDomain z)) <$> execQuerySpanThrow BlogPosts.getBlogPostsWithUser
...
filterByAuthor :: DisplayName -> [(BlogPosts.Domain, User.Domain)] -> [(BlogPosts.Domain, User.Domain)]
filterByAuthor name = filter (((== name) . User.dDisplayName) . snd)

filterPosts :: SearchQuery -> [(BlogPosts.Domain, User.Domain)] -> [(BlogPosts.Domain, User.Domain)]
...
filterByStartDate :: StartDate -> [(BlogPosts.Domain, User.Domain)] -> [(BlogPosts.Domain, User.Domain)]
filterByStartDate (StartDate start) =
  filter $ \(BlogPosts.Domain {..}, _) -> maybe False ((>= start) . utctDay) dPublishedAt

filterByEndDate :: EndDate -> [(BlogPosts.Domain, User.Domain)] -> [(BlogPosts.Domain, User.Domain)]
