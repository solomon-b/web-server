  ...
  where
    ...
    getAuth cfg =
      bracketConn cfg $ do
        ...
        let email = mkEmailAddress "user@host.com"
            displayName = mkDisplayNameUnsafe "user"
            fullName = mkFullNameUnsafe "fullUserName"
            isAdmin = True
        auth <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
          uid <- TRX.statement () $ User.insertUser $ User.ModelInsert email pass
          _ <- TRX.statement () $ UserMetadata.insert $ UserMetadata.ModelInsert (getOneRow uid) displayName fullName Nothing isAdmin
          ...
          let fullUser = FullUser (getOneRow uid) email displayName fullName Nothing isAdmin
          pure $ Authz fullUser (Session.toDomain $ getOneRow sm)
        case auth of
