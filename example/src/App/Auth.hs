  ...
  let eSession = do
   ...
   in do
        case eSession of
          ...
          Right sessionId ->
            liftIO (HSQL.use pool (HSQL.statement () $ UserQuery.getSessionFullUser sessionId)) >>= \case
              ...
              -- Auth Success
              Right (Just (fullUser, sessionModel)) ->
                pure $ Authz fullUser (Session.toDomain sessionModel)
...
getAuth sessionId = do
  eSessionData <- runDB . HSQL.statement () $ UserQuery.getSessionFullUser sessionId
  pure $
    eSessionData
      <&> fmap
        ( \(fullUser, serverSession) ->
            Authz
              { authzUser = fullUser,
...
-- | Login state using FullUser for display purposes
data LoggedIn = IsLoggedIn FullUser | IsNotLoggedIn
