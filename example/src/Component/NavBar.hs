...
userDropdown :: FullUser -> Lucid.Html ()
userDropdown FullUser {fuId, fuDisplayName, fuAvatarUrl} =
  let avatar = fromMaybe [i|/#{staticGetUrl}/avatar.png|] fuAvatarUrl
   in div_ do
        button_ [id_ "adminUserButton", hxGet_ [i|/user/#{display fuId}|], hxTarget_ "#main", hxPushUrl_ "true", dataDropdownToggle_ "adminUserDropdown", dataDropdownOffsetDistance_ "0", dataDropdownTrigger_ "hover", class_ "font-medium text-sm p-2.5 text-center inline-flex items-center", type_ "button"] do
          img_ [src_ avatar, class_ "size-4"]
          span_ [class_ "ps-2"] do
            [i|Hello, #{display fuDisplayName}|]
...
adminNavBar = \case
  (Auth.IsLoggedIn user@FullUser {fuIsAdmin})
    | fuIsAdmin ->
