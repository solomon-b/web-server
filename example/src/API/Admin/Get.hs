...
userRow :: FullUser -> Lucid.Html ()
userRow FullUser {..} =
  tr_ [class_ "bg-white border-b"] $ do
    th_ [scope_ "row", class_ "px-6 py-4 font-medium text-gray-900 whitespace-nowrap"] (Lucid.toHtml $ display fuId)
    td_ [class_ "px-6 py-4"] (Lucid.toHtml $ display fuEmail)
    td_ [class_ "px-6 py-4"] (Lucid.toHtml $ display fuDisplayName)
    td_ [class_ "px-6 py-4"] (Lucid.toHtml $ display fuAvatarUrl)
    td_ [class_ "px-6 py-4"] (Lucid.toHtml $ display fuIsAdmin)

userTable :: [FullUser] -> Lucid.Html ()
userTable entries =
