    ...
    getConfig >>= \case
      ...
      Just AppConfig {..} -> do
        ...
        Observability.withTracer appConfigObservability $ \tracerProvider mkTracer -> do
          ...
          Warp.runSettings warpSettings (otelMiddleware $ mkApp proxy server servantContext appContext)
...
mkApp ::
  ...
  AppContext ctx ->
  Servant.Application
mkApp proxy server cfg ctx =
  Servant.serveWithContext proxy cfg $
